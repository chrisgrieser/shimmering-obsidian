...
- Press `↵` to open the plugin in Obsidian's Community Plugin Browser.
	...
	- _⚙️ For developers:_ Use `fn + ↵` to clone the GitHub Repository into the folder specified in the [workflow configuration](Workflow%20Configuration.md#Plugin-and-Theme-Search) (`download_folder_path`) via `git clone`.
...
- Use `opt + ↵` on an issue to copy the issue URL to the clipboard.
