...
## Alfred Commands
- `ovault`: You can also select which Vault to use with the [`ovault` command](Utility%20features.md#Change-and-Access-your-Vault). Even if you have only one vault, you need to select that one vault once before you can use the workflow.
...
- `vault_path`: The *absolute* path to your obsidian vault, e.g., `/Users/pseudometa/MyVault` or `~/Documents/obsidianVault`. (💡 You do not need to set it manually, the [`ovault` command](#Alfred-Commands) will set that value for you already.)
