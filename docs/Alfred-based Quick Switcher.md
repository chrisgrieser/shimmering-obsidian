...
🆕 The `o`-search will respect the `Excluded Files` setting from Obsidian. (ℹ️ Note that an Obsidian restart is required after changing the `Excluded Files` before Alfred can pick up the changes.)
